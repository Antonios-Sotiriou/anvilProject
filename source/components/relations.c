...
int addChild(mesh *t, mesh *m) {
	...
	if (m->parent) {
		...
		for (int i = 0; i < m->number_of_children; i++) {
			if (strncmp(t->cname, m->children[i].cname, strlen(m->children->cname)) == 0) {
	...
	if (t->number_of_children == 0) {
		t->children = malloc(sizeof(mesh));
		if (t->children) {
			t->children[0] = *m;
		}
		t->number_of_children = 1;
	} else {
		mesh *temp_t = realloc(t->children, sizeof(mesh) * (t->number_of_children + 1));
		if (!temp_t) {
			free(t->children);
...
int removeChild(mesh *t, mesh *m) {
	...
	if (t->number_of_children > 0) {
		...
		size_t ch_size = mesh_size * t->number_of_children;
