...
void moveForward(const int action) {
    if (SCENE.model[camera].rigid.state) {
        SCENE.model[camera].rigid.velocity = vecMulf32(vecMulf32(SCENE.model[camera].coords.v[3], FORCE), action);
        return;
    }
    velocity = vecMulf32(vecMulf32(SCENE.model[camera].coords.v[3], (FORCE * SCALE_NR)), action);
    SCENE.model[camera].coords.v[0] = vecAddvec(SCENE.model[camera].coords.v[0], velocity);
    memcpy(&SCENE.model[camera].mesh[0].coords, &SCENE.model[camera].coords, 64);
}
...
void moveRight(const int action) {
    if (SCENE.model[camera].rigid.state) {
        SCENE.model[camera].rigid.velocity = vecMulf32(vecMulf32(SCENE.model[camera].coords.v[1], FORCE), action);
        return;
    }
    velocity = vecMulf32(vecMulf32(SCENE.model[camera].coords.v[1], (FORCE * SCALE_NR)), action);
    SCENE.model[camera].coords.v[0] = vecAddvec(SCENE.model[camera].coords.v[0], velocity);
}
...
void moveUp(const int action) {
    if (SCENE.model[camera].rigid.state) {
        SCENE.model[camera].rigid.velocity = vecMulf32(vecMulf32(SCENE.model[camera].coords.v[2], FORCE), action);
        return;
    }
    velocity = vecMulf32(vecMulf32(SCENE.model[camera].coords.v[2], (FORCE * SCALE_NR)), action);
    SCENE.model[camera].coords.v[0] = vecAddvec(SCENE.model[camera].coords.v[0], velocity);
}
...
void lookRight(const int action) {
    ...
    mat4x4 tr = matfromQuat(rotationQuat(-10.f * action, 0.f, 1.f, 0.f), SCENE.model[camera].coords.v[0]);
    setvec4arrayMulmat(SCENE.model[camera].coords.v, 4, tr);
}
...
void lookUp(const int action) {
    ...
    mat4x4 tr = matfromQuat(rotationQuat(10.f * action, vec4ExtractX(SCENE.model[camera].coords.v[1]), vec4ExtractY(SCENE.model[camera].coords.v[1]), vec4ExtractZ(SCENE.model[camera].coords.v[1])), SCENE.model[camera].coords.v[0]);
    setvec4arrayMulmat(SCENE.model[camera].coords.v, 4, tr);
}
