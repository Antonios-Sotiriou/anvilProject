...
static int initIndividualTerrainHeightMap(TerrainInitInfo *tif) {
    ...
    sprintf_s(path, 100, "height_maps/%s/%s%dx%d.bmp", tif->cname, tif->cname, tif->width, tif->height);
#elif defined(LINUX) || defined(__linux__)
    snprintf(path, 100, "height_maps/%s/%s%dx%d.bmp", tif->cname, tif->cname, tif->width, tif->height);
#endif
    ...
    char filepath[100] = { 0 };
#if defined(WIN32) || defined(_WIN32) || defined(_WIN64)
    sprintf_s(filepath, 100, "%s/models/%s/%s.obj", anvil_SOURCE_DIR, tif->cname, tif->cname);
#elif defined(LINUX) || defined(__linux__)
    snprintf(filepath, 100, "%s/models/%s/%s.obj", anvil_SOURCE_DIR, tif->cname, tif->cname);
#endif
...
static void initTerrainFaces(BMP* bmp, const int num_of_faces, const char new_file_path[]) {
    ...
    for (int x = 0; x < num_of_faces; x += 18) {
        ...
        /* Face 1st Up. */
        #if defined(WIN32) || defined(_WIN32) || defined(_WIN64)
            sprintf_s(data_1, 90, "f %d/%d/%d %d/%d/%d %d/%d/%d\n", face_1_0, face_1_0, 0, face_1_1, face_1_1, 0, face_1_2, face_1_2, 0);
        #elif defined(LINUX) || defined(__linux__)
            snprintf(data_1, 90, "f %d/%d/%d %d/%d/%d %d/%d/%d\n", face_1_0, face_1_0, 0, face_1_1, face_1_1, 0, face_1_2, face_1_2, 0);
        #endif
        ...
        /* Face 2nd Down. */
        #if defined(WIN32) || defined(_WIN32) || defined(_WIN64)
            sprintf_s(data_2, 90, "f %d/%d/%d %d/%d/%d %d/%d/%d\n", face_1_0, face_1_0, 0, face_1_2, face_1_2, 0, face_1_0 + 1, face_1_0 + 1, 0);
        #elif defined(LINUX) || defined(__linux__)
            snprintf(data_2, 90, "f %d/%d/%d %d/%d/%d %d/%d/%d\n", face_1_0, face_1_0, 0, face_1_2, face_1_2, 0, face_1_0 + 1, face_1_0 + 1, 0);
        #endif
...
static void updateTerrainQuadsInfoDB(BMP* bmp, const char cname[]) {
    ...
    char command[150] = { 0 };
#if defined(WIN32) || defined(_WIN32) || defined(_WIN64)
    sprintf_s(command, 150, "UPDATE terrain SET quad_cols = %d, quad_rows = %d, quads_indexes = %d WHERE cname = '%s';", quad_cols, quad_rows, quads_indexes, cname);
#elif defined(LINUX) || defined(__linux__)
    snprintf(command, 150, "UPDATE terrain SET quad_cols = %d, quad_rows = %d, quads_indexes = %d WHERE cname = '%s';", quad_cols, quad_rows, quads_indexes, cname);
#endif
